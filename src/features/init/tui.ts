...
export function parseShellFile(filePath: string): Record<string, string> {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      const blackList = ['PATH', 'HOME', 'USER', 'SHELL', 'EDITOR', 'LANG', 'PWD', 'OLDPWD', 'ZSH', 'SHLVL'];
...
export async function runTui(providedFile?: string): Promise<{ selectedEnv: Record<string, string> }> {
  ...
  const secretKeywords = ['KEY', 'API', 'SDK', 'SECRET', 'TOKEN', 'PASSWORD', 'AUTH', 'CREDENTIAL', 'PRIVATE'];
  
  // Sort and Pre-identify likely secrets
  ...
  for (const k of keys) {
    if (secretKeywords.some(kw => k.toUpperCase().includes(kw))) {
      likelySecretKeys.push(k);
    }
  }

  const varSelection = await p.multiselect({
    message: Flexoki.tx('Select variables to move into Vault (Likely secrets pre-checked):'),
    options: sortedKeys.map(k => {
      const isLikelySecret = likelySecretKeys.includes(k);
      return { 
        value: k, 
        label: k + (isLikelySecret ? Flexoki.tx2(' (likely secret)') : ''),
      };
    }),
    initialValues: likelySecretKeys, // THIS MAKES IT FAST
    required: true,
  });
